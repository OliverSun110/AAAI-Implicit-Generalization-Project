...
class Trainer(object):
    ...
    def train(self, train_iter_fct, valid_iter_fct, train_steps, valid_steps):
        ...
        valid_stats = self.validate(valid_iter)
        valid_stats = self._maybe_gather_stats(valid_stats)
        with open('./../output_batch_'+batch.batch_size+'.txt','a+') as f:
            f.write('Train perplexity: %g' % report_stats.ppl())
            f.write('\n')
            f.write('Train accuracy: %g' % report_stats.accuracy())
            f.write('\n')
            f.write('Valid perplexity: %g' % valid_stats.ppl())
            f.write('\n')
            f.write('Valid accuracy: %g' % valid_stats.accuracy())
            f.write('\n')
        return total_stats
